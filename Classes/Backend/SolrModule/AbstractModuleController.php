...
abstract class AbstractModuleController extends ActionController implements AdministrationModuleInterface {
	...
	protected function initializeAction() {
		...
		if (!$this->site instanceof \Tx_Solr_Site) {
			$fallBackRootPageId = $this->getFallBackRootPageId();
			$this->site = \Tx_Solr_Site::getSiteByPageId($fallBackRootPageId);
		}
	...
	/**
	 * Get first rootpage id
	 *
	 * @return int
	 */
	protected function getFallBackRootPageId() {
		$rootPageId = 0;
		...
		if (count($rootPages) > 0) {
			$rootPageId = $rootPages[0]['uid'];
		}

		return (int) $rootPageId;
	}
}
